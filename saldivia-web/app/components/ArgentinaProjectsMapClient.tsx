        ...
        <div className="grid items-start gap-10 lg:items-stretch lg:grid-cols-[minmax(320px,0.78fr)_minmax(0,1.22fr)] xl:gap-12">
            ...
            <div className="relative lg:sticky lg:top-28">
              ...
              <div className="mb-8 grid gap-3 sm:grid-cols-3">
                <div className="rounded-xl border border-cyan-400/35 bg-[#081a30]/85 px-4 py-4 text-center font-headline sm:text-left">
                  <p className="text-xl font-black tabular-nums text-white">{aggregate.provincesWith}</p>
                  <p className="mt-1 text-[10px] font-black uppercase tracking-[0.26em] text-slate-400">
                    Provincias
                  </p>
                </div>
                <div className="rounded-xl border border-cyan-400/35 bg-[#081a30]/85 px-4 py-4 text-center font-headline sm:text-left">
                  <p className="text-xl font-black tabular-nums text-white">{aggregate.projectsTotal}</p>
                  <p className="mt-1 text-[10px] font-black uppercase tracking-[0.26em] text-slate-400">
                    Proyectos
                  </p>
                </div>
                <div className="rounded-xl border border-cyan-400/35 bg-[#081a30]/85 px-4 py-4 text-center font-headline sm:text-left">
                  <p className="text-xl font-black tabular-nums text-white">+25</p>
                  <p className="mt-1 text-[10px] font-black uppercase tracking-[0.26em] text-slate-400">Años</p>
                </div>
              </div>

              <div className="relative rounded-[1.5rem] border border-cyan-400/35 bg-[#081a30]/92 p-5 shadow-[0px_36px_80px_rgba(0,10,26,0.55)] backdrop-blur-sm md:p-6">
                <div
                  ref={panelAnchorRef}
                  aria-hidden
                  className="absolute right-0 top-1/2 z-[1] hidden h-28 w-px -translate-y-1/2 translate-x-px lg:block"
                />

                <div className="mb-5 flex flex-wrap items-center gap-2">
                  <span className="inline-flex items-center rounded-full bg-accent-blue px-2.5 py-1 text-[10px] font-black uppercase tracking-[0.22em] text-white shadow-[0_0_22px_rgba(32,149,212,0.35)]">
                    Provincia activa
                  </span>
                  <span className="inline-flex items-center rounded-full border border-white/18 bg-[#071422]/85 px-2.5 py-1 text-[10px] font-bold uppercase tracking-[0.18em] text-slate-300">
                    {activeProjects.length} destacado
                    {activeProjects.length === 1 ? "" : "s"}
                  </span>
                  <span className="inline-flex items-center rounded-full border border-white/12 px-2.5 py-1 text-[10px] font-bold uppercase tracking-[0.18em] text-slate-500">
                    {selectionLocked ? "Selección fija" : "Hover activo"}
                  </span>
                  {selectionLocked ? (
                    <button
                      className="inline-flex items-center rounded-full border border-cyan-400/40 px-2.5 py-1 text-[10px] font-bold uppercase tracking-[0.18em] text-accent-blue transition-colors hover:bg-cyan-400/15"
                ...
                <div className="mb-5">
                  <h3 className="font-headline text-2xl font-extrabold text-white md:text-[2rem]">
                    {activeProvince.name}
                  </h3>
                ...
                <div className="space-y-3">
                  {activeProjects.length > 0 ? (
                    <>
                      ...
                      {activeProject ? (
                        <article
                          ...
                          className="relative overflow-hidden rounded-[1.25rem] border border-white/14 bg-[#051018]/92 pl-[4px]"
                        >
                          <div
                          ...
                          <div className="p-4 pl-[1.125rem]">
                            <div className="mb-2.5 flex flex-wrap items-center gap-2">
                              <span className="inline-flex items-center rounded-full border border-accent-blue/50 bg-accent-blue/20 px-2.5 py-1 text-[9px] font-black uppercase tracking-[0.22em] text-accent-blue">
                                {activeProject.segment}
                              </span>
                              {activeProject.year.trim() && activeProject.year.trim() !== "—" ? (
                                <span className="inline-flex items-center rounded-full border border-white/15 px-2.5 py-1 text-[9px] font-bold uppercase tracking-[0.18em] text-slate-500">
                                  {activeProject.year}
                                </span>
                              ) : null}
                            </div>
                            <h4 className="font-headline text-lg font-bold text-white">{activeProject.title}</h4>
                    ...
                    <div className="rounded-[1.25rem] border border-dashed border-white/22 bg-[#071422]/80 p-5 text-[13px] leading-relaxed text-slate-500">
          ...
          <div className="relative order-1 lg:order-2">
            ...
            >
              ...
              {/* Contador */}
              <div className="absolute right-4 top-4 z-[6] rounded-full border border-cyan-400/40 bg-[#061220]/94 px-3 py-1.5 font-headline text-[10px] font-bold uppercase tracking-[0.15em] text-slate-200 shadow-[0_0_26px_rgba(32,149,212,0.15)] backdrop-blur-sm md:right-6 md:top-6">
              ...
              (projects[hoveredProvinceId] ?? []).length > 0 ? (
                ...
                >
                  <p className="font-headline text-[9px] font-black uppercase tracking-[0.26em] text-accent-blue">
                    {mapTooltip.segment}
                  </p>
                  <p className="mt-1.5 font-headline text-[13px] font-bold leading-tight text-white">
                    {mapTooltip.title}
                  </p>
              ...
              {/* Leyenda */}
              <div className="relative z-[2] mt-4 flex flex-wrap items-center justify-center gap-6 border-t border-white/10 pt-4 text-[10px] font-bold uppercase tracking-[0.16em] text-slate-400 md:justify-end">
